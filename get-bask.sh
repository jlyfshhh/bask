...
reject_unsafe_path_text() {
  ...
  case "$value" in
    ...
    *$'\n'*|*$'\r'*) die "$label cannot contain line breaks." ;;
    \'*|\"*|*\'|*\") die "$label must be an unquoted path value; quote characters at the edges are ambiguous." ;;
...
validate_candidate() {
  ...
  sh -n "$candidate/dbus-proxy-entrypoint.sh" ||
    die "The downloaded D-Bus proxy entrypoint did not pass syntax validation."
}
