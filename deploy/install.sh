...
install_path_is_protected() {
  local path="$1"
  case "$path" in
    /|/bin|/bin/*|/boot|/boot/*|/dev|/dev/*|/etc|/etc/*|/lib|/lib/*|/lib64|/lib64/*|/private/etc|/private/etc/*|/private/tmp|/private/tmp/*|/private/var|/private/var/*|/proc|/proc/*|/run|/run/*|/sbin|/sbin/*|/sys|/sys/*|/tmp|/tmp/*|/usr|/usr/*|/var|/var/*|/home|/mnt|/opt|/root|/srv|"${run_home:-/nonexistent}") return 0 ;;
  esac
  return 1
}
...
services=(bask bask-scanner bask-dbus-proxy)
...
rollback_install() {
  ...
  restore_file "$data_path/config.json" app-config || failed=true
    ...
    if [[ ! -f "$state/compose.present" ]]; then
      ...
      for service in "${services[@]}"; do
        if [[ -f "$state/$service.present" ]]; then
          ...
          previous_compose rm -sf "$service" >/dev/null 2>&1 || failed=true
        fi
      done
    fi
  fi
...
path_is_protected() {
  local path="$1"
  case "$path" in
    /|/bin|/bin/*|/boot|/boot/*|/dev|/dev/*|/etc|/etc/*|/lib|/lib/*|/lib64|/lib64/*|/private/etc|/private/etc/*|/private/tmp|/private/tmp/*|/private/var|/private/var/*|/proc|/proc/*|/run|/run/*|/sbin|/sbin/*|/sys|/sys/*|/tmp|/tmp/*|/usr|/usr/*|/var|/var/*|/home|/mnt|/opt|/root|/srv|"${run_home:-/nonexistent}"|"$project_dir") return 0 ;;
  esac
  return 1
}
...
verify_backup_archive() {
  local archive="$1" source_data="$2" verify_dir saved_config saved_db check
...
if [[ "$legacy" == true ]]; then
  ...
  if [[ ! -f "$data_path/readings.db" && -f "$project_dir/readings.db" ]]; then
    sqlite_destination="$data_path/readings.db"
    sqlite_quoted="${sqlite_destination//\'/\'\'}"
    sqlite3 "$project_dir/readings.db" ".backup '$sqlite_quoted'"
...
verify_container_boundary() {
  local web_mounts scanner_mounts proxy_mounts scanner_user proxy_user
  local scanner_read_only proxy_read_only host_bus_source_pattern proxy_bus_pattern
  web_mounts="$(docker inspect --format '{{range .Mounts}}{{printf "%s|%s\n" .Source .Destination}}{{end}}' bask 2>/dev/null || true)"
  scanner_mounts="$(docker inspect --format '{{range .Mounts}}{{printf "%s|%s\n" .Source .Destination}}{{end}}' bask-scanner 2>/dev/null || true)"
  proxy_mounts="$(docker inspect --format '{{range .Mounts}}{{printf "%s|%s\n" .Source .Destination}}{{end}}' bask-dbus-proxy 2>/dev/null || true)"
  scanner_user="$(docker inspect --format '{{.Config.User}}' bask-scanner 2>/dev/null || true)"
  proxy_user="$(docker inspect --format '{{.Config.User}}' bask-dbus-proxy 2>/dev/null || true)"
  scanner_read_only="$(docker inspect --format '{{.HostConfig.ReadonlyRootfs}}' bask-scanner 2>/dev/null || true)"
  proxy_read_only="$(docker inspect --format '{{.HostConfig.ReadonlyRootfs}}' bask-dbus-proxy 2>/dev/null || true)"
  host_bus_source_pattern='^/(var/)?run/dbus/system_bus_socket\|'
  proxy_bus_pattern='^/(var/)?run/dbus/system_bus_socket\|/host/run/dbus/system_bus_socket$'
  ! grep -Eq "$host_bus_source_pattern" <<<"$web_mounts" &&
    ! grep -Eq "$host_bus_source_pattern" <<<"$scanner_mounts" &&
    grep -Eq "$proxy_bus_pattern" <<<"$proxy_mounts" &&
    [[ -n "$scanner_user" && "$scanner_user" != 0 && "$scanner_user" != 0:* ]] &&
    [[ "$proxy_user" == 0 || "$proxy_user" == 0:* ]] &&
    [[ "$scanner_read_only" == true && "$proxy_read_only" == true ]]
}
...
for _ in $(seq 1 "$health_attempts"); do
  ...
  proxy_state="$(docker inspect --format '{{.State.Status}}' bask-dbus-proxy 2>/dev/null || true)"
  proxy_health="$(docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}' bask-dbus-proxy 2>/dev/null || true)"
  proxy_oom="$(docker inspect --format '{{.State.OOMKilled}}' bask-dbus-proxy 2>/dev/null || true)"
  if [[ "$health" == healthy && "$web_oom" != true &&
        "$scanner_state" == running && "$scanner_health" == healthy && "$scanner_oom" != true &&
        "$proxy_state" == running && "$proxy_health" == healthy && "$proxy_oom" != true ]]; then
    healthy=true
    break
  fi
  if [[ "$health" == unhealthy || "$health" == exited || "$health" == dead || "$web_oom" == true ||
        "$scanner_health" == unhealthy || "$scanner_state" == exited || "$scanner_state" == dead || "$scanner_oom" == true ||
        "$proxy_health" == unhealthy || "$proxy_state" == exited || "$proxy_state" == dead || "$proxy_oom" == true ]]; then
    break
  fi
...
if [[ "$healthy" != true ]]; then
  candidate_compose logs --tail=80 >&2 || true
  fail_with_rollback "Bask did not become healthy (web=${health:-missing}, scanner=${scanner_health:-missing}, proxy=${proxy_health:-missing})."
fi
