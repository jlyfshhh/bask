...
reject_path_text() {
  ...
  case "$value" in
    ...
    *\'*|*\"*) echo "$label cannot contain quote characters; Docker Compose and this tool do not parse them identically." >&2; exit 1 ;;
...
[[ ! -L "$root/.env" ]] || { echo "Refusing a symlinked Bask environment file: $root/.env" >&2; exit 1; }
...
[[ ! -f "$root/.env" ]] || copy_private "$root/.env" "$work/.env"
...
if [[ -f "$data_dir/readings.db" ]]; then
  ...
  if [[ -r "$data_dir/readings.db" ]]; then
    # Keep the CLI dot-command destination deliberately simple. sqlite3's
    # .backup parser is not SQL parameter binding, so paths containing quotes
    # cannot be made reliable by SQL-style quote doubling.
    (cd -- "$work" && sqlite3 "$data_dir/readings.db" '.backup readings.db')
  elif command -v sudo >/dev/null 2>&1; then
    (cd -- "$work" && sudo sqlite3 "$data_dir/readings.db" '.backup readings.db')
