...
function dialogFocusable(dialog) {
  ...
  )].filter(node => !node.closest("[inert]") && node.getAttribute("aria-hidden") !== "true");
}
...
function syncDialogBackground() {
  ...
  for (const node of document.body.children) {
    if (!(node instanceof HTMLElement) || node === top || node.id === "toast" || ["SCRIPT", "STYLE", "LINK"].includes(node.tagName)) continue;
    _dialogBackground.set(node, { inert: node.inert, ariaHidden: node.getAttribute("aria-hidden") });
    node.inert = true;
...
function openDialog(id, focusSelector) {
  ...
  const target = focusSelector ? dialog.querySelector(focusSelector) : dialog;
...
function alertStatusTime(epoch) {
  if (!Number.isFinite(Number(epoch))) return "never";
...
function alertDeliveryMarkup(status) {
  if (!status) return "";
  const pending = Number.isInteger(status.pending) ? status.pending : 0;
  const pendingText = pending
    ? `<b>${pending}</b> alert${pending === 1 ? " is" : "s are"} pending${status.retrying ? " and retrying automatically" : ""}.`
    : "No alerts are waiting to send.";
  const success = `Last delivered: <b>${esc(alertStatusTime(status.last_success_at))}</b>`;
  const error = status.last_error_at
    ? `Last error: <b>${esc(alertStatusTime(status.last_error_at))}</b>${status.last_error ? ` — ${esc(status.last_error)}` : ""}`
    : "Last error: <b>none</b>";
  return `<div class="ntfy-delivery" role="status">
    <div>${pendingText}</div><div>${success}</div><div>${error}</div>
  </div>`;
}
...
async function refreshAlertsUI() {
  ...
  if (!el) return;
  try {
    [_ntfy, _ntfyDelivery] = await Promise.all([
      api("GET", "/api/ntfy"), api("GET", "/api/ntfy/delivery")
    ]);
  } catch (e) { el.innerHTML = ""; return; }
  if (!_ntfy.enabled) {
