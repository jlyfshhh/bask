...
class KeeperUnlockThrottle:
    ...
    def check(self, source: str | None, key_fingerprint: str) -> int:
        with self._lock:
            now = self._clock()
            waits = [
                self._global.retry_after(_GLOBAL_KEY, now),
                self._keys.retry_after(key_fingerprint, now),
            ]
            if source:
    ...
    def fail(self, source: str | None, key_fingerprint: str) -> int:
        with self._lock:
            now = self._clock()
            waits = [
                self._global.fail(_GLOBAL_KEY, now),
                self._keys.fail(key_fingerprint, now),
            ]
            if source:
...
def key_fingerprint(key: str) -> str:
    """Return a process-private identity for one exact submitted key."""
