...
configuration file. Transition functions are pure; ``AlertStateStore`` is the
small persistence and concurrency boundary around them.
...
that narrow interval can duplicate an alert, but cannot lose it.
"""
...
def _new_pending(
    state: dict[str, Any], enclosure_id: str, target: str,
    message: dict[str, str], now: float,
) -> dict[str, Any]:
...
def observe(
    state: dict[str, Any], observations: Iterable[dict[str, Any]], *,
    enabled: bool, now: float, debounce_seconds: int = DEBOUNCE_SECONDS,
) -> dict[str, Any]:
    ...
    for enclosure_id, item in current.items():
        ...
        if pending is not None and pending.get("target") != group:
            # The outbox describes a state that is no longer true. Cancel it
            # immediately instead of allowing a failed delivery to retry
            # during the debounce window for the new state. Checking this on
            # every observation also repairs a stale retry loaded from a state
            # file written by an older Bask version.
            #
            # Baseline only advances after a successful delivery, so an unsent
            # warning that recovers produces no misleading recovery-only ping.
            entry["pending"] = None
        ...
        if pending and pending.get("target") == group:
            # Preserve retry identity and timing while refreshing the text.
...
def delivery_failed(state: dict[str, Any], event_id: str, *, now: float) -> dict[str, Any]:
    ...
    for entry in out.get("enclosures", {}).values():
        ...
        # urllib errors can contain the private topic URL; never persist them.
...
def _normalise_loaded(raw: Any) -> dict[str, Any]:
    ...
    for enclosure_id, source in entries.items():
        ...
        if isinstance(pending, dict):
            ...
            if (
                isinstance(event_id, str) and len(event_id) <= 256
...
class AlertStateStore:
    ...
    def _load(self) -> dict[str, Any]:
        try:
            ...
            # Fail closed to a fresh baseline, never infer a new alert.
            state = initial_state()
    ...
    def observe(
        self, observations: Iterable[dict[str, Any]], *, enabled: bool,
        now: float, debounce_seconds: int = DEBOUNCE_SECONDS,
    ) -> None:
        with self._lock:
            self._replace(observe(
                self._state, observations, enabled=enabled, now=now,
