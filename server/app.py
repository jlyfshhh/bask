...
# Bask is an appliance UI, not an API-development host. Leaving FastAPI's
# interactive documentation and schema endpoints enabled publishes a complete
# route inventory to every device on the LAN and adds script-heavy pages that
...
# The frontend predates a bundler and still uses inline event-handler
# attributes generated by frontend/app.js. Consequently `script-src` cannot
# yet drop `unsafe-inline` without breaking every setup editor. The policy is
# still useful: it limits scripts, connections, media, and workers to Bask's
# own origin, prevents plugins and framing, and leaves that one residual
# explicitly documented rather than pretending the CSP is stricter than it is.
...
async def _notify_loop():
    ...
    The first enabled pass only seeds a durable baseline, so starting Bask or
    opting in never creates a burst for pre-existing conditions.  A failed send
    remains in the owner-only outbox across retries and process restarts.
    """
    while True:
        try:
            cfg = load_config()
            ...
            if enabled:
                # Bound time spent in one cycle.  Each enclosure has at most one
                # pending event and failed events move into the future, so this
                # also gives newer transitions a fair chance during an outage.
                for _ in range(MAX_DELIVERIES_PER_CYCLE):
                    ...
                    try:
                        ...
                        # urllib exceptions may contain the private topic URL;
                        # never put that exception text into logs or status.
...
def ntfy_delivery_status(_: None = Keeper):
    """Safe keeper-only health for the durable delivery outbox.

    This response intentionally carries no server, topic, URL, message body, or
    enclosure name, so it is useful for diagnosis without becoming a secret.
    """
    cfg = load_config()
    enabled = bool(cfg["ntfy"].get("enabled") and cfg["ntfy"].get("topic"))
...
def ntfy_set(payload: NtfyToggle, _: None = Keeper, revision: int = ConfigWrite):
    ...
    # A genuine off→on transition must seed from the next current snapshot,
    # never reuse an old baseline and announce changes that occurred while off.
...
def ntfy_test(_: None = Keeper):
    ...
    try:
        ...
        # urllib exceptions commonly include the complete destination URL,
        # whose final path component is the private ntfy topic.  Keep that
        # credential out of both the API response and routine logs.
